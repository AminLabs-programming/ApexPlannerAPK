...
// Read site config defensively instead of a static `import ... from './.figma/make/site.json'`.
// A static JSON import makes the whole build fail if this file is ever missing
// (e.g. excluded from a zip export or .gitignore'd since it starts with a dot).
// Falling back to defaults keeps `pnpm run build` working even without it.
...
function loadSiteConfiguration(): FigmaSiteConfiguration {
  ...
  try {
    ...
    return { ...DEFAULT_SITE_CONFIGURATION, ...JSON.parse(raw) }
  } catch {
    console.warn(
      `[vite.config] './.figma/make/site.json' not found or unreadable — using default site configuration.`,
    )
